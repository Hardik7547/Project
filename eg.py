...
def pred_price():
    ...
    if seller_selected_value == "Dealer":
        input_values.extend([1.0, 0.0, 0.0])
    elif seller_selected_value == "Individual":
        input_values.extend([0.0, 1.0, 0.0])
    else:
        input_values.extend([0.0, 0.0, 1.0])

    if fuel_selected_value == "CNG":
        input_values.extend([1.0, 0.0, 0.0, 0.0])
    elif fuel_selected_value == "Diesel":
        input_values.extend([0.0, 1.0, 0.0, 0.0])
    elif fuel_selected_value == "LPG":
        input_values.extend([0.0, 0.0, 1.0, 0.0])
    else:
        input_values.extend([0.0, 0.0, 0.0, 1.0])

    if transmission_selected_value == "Automatic":
        input_values.extend([1.0, 0.0])
    else:
        input_values.extend([0.0, 1.0])

    prediction = model.predict([input_values])[0]
    prediction_label.config(text=f"Predicted Price: {prediction}")
...
def save_to_csv(prediction):
    ...
    with open(filename, 'a', newline='') as csvfile:
        fieldnames = ['Predicted Price']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        
        if not file_exists:
            writer.writeheader()
        
        writer.writerow({'Predicted Price': prediction})
...
prediction_label = Label(root, text="", bg="black", fg="white", font=("Arial", 20, "bold"))
